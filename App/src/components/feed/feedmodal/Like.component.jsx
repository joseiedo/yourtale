import {useLikePost} from "../../../hooks/post/useLikePosts.hook";
import {useUnlikePost} from "../../../hooks/post/useUnlikePosts.hook";
import styles from "./FeedModal.module.css";
import React from "react";

export function Like({isLiked, likesQuantity, token, postId, getPostDetails}) {
    const {likePost} = useLikePost();
    const {unlikePost} = useUnlikePost();

    async function handlePostInteract() {
        if (isLiked) {
    ...
    const HeartOutlineSvg = () => <svg clip-rule="evenodd" fill-rule="evenodd" stroke-linejoin="round"
                                       stroke-miterlimit="2"
    ...
    const FilledSvg = () => <svg clip-rule="evenodd" fill-rule="evenodd" stroke-linejoin="round" stroke-miterlimit="2"
                                 viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
        <path
    ...
    return <div className={styles.like} onClick={handlePostInteract}>    {
        isLiked ? <FilledSvg/> :
            <HeartOutlineSvg/>
    }

        <p>{likesQuantity} {likesQuantity === 1 ? 'curtida' : 'curtidas'}</p>
    </div>
